...
class ExternalWorld:
	...
	def tick(self):
		...
		if self.time < self.const.LEN_WEEK*self.const.LEN_HOUR*self.const.LEN_MIN and not(self.restart_flag):
			...
			if self.time_befor_new_client-1 <= 0 and self.hour >=self.const.START_WORK_DAY and self.hour < self.const.END_WORK_DAY and not(self.day==self.const.SHORT_WORK_DAY and self.hour >= self.const.END_SHORT_DAY)	and not(self.day==self.const.NOT_WORK_DAY) :
			...
			#if working hours
			if self.hour >=self.const.START_WORK_DAY and self.hour < self.const.END_WORK_DAY and not(self.day==self.const.SHORT_WORK_DAY and self.hour >= self.const.END_SHORT_DAY)	and not(self.day==self.const.NOT_WORK_DAY) and self.bank.free_clerks!=[]:
	...
	def what_is_coef_speed_time(self):
		...
		if self.hour == self.const.HOUR_WHEN_LINES_CLIENTS_WILL_MORE and not(self.day==self.const.NOT_WORK_DAY):
			coef = self.const.COEF_NORM_TIME_SPEED
		elif self.hour < self.const.START_WORK_DAY or self.hour > self.const.END_WORK_DAY or self.day==self.const.SHORT_WORK_DAY and self.hour == self.const.END_SHORT_DAY or self.day==self.const.NOT_WORK_DAY:
			coef = 0
		...
		if self.day_num==self.last_day_num or self.day==self.const.SHORT_WORK_DAY and self.hour == END_SHORT_DAY or self.day==self.const.NOT_WORK_DAY:
			return coef
